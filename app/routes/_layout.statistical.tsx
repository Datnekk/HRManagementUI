import { Tabs, Card, Group, Text, Badge, ThemeIcon, Box, Progress, Grid, Button, Select } from "@mantine/core";
import {
  LoaderFunctionArgs,
} from "@remix-run/node";
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (userAttendanceError) {
    ...
    throw new Error(`Failed to load attendance data: ${userAttendanceError.message}`);
  }
  ...
  if (salaryResult?.data && !Array.isArray(salaryResult?.data) && typeof salaryResult?.data === 'object') {
    salaryData = salaryResult.data.salaries || salaryResult.data.items || salaryResult.data.result || salaryResult.data;
  }
  ...
  let currentWork = 0, currentOT = 0;
  let lastWork = 0, lastOT = 0;
  ...
  if (Array.isArray(salaryData)) {
    // Nếu là array
    salaryChartData = salaryData.map((salary: any) => {
      ...
      return {
        ...
        Total: (salary.BaseSalary || 0) + (salary.Allowances || 0) + (salary.Bonus || 0)
      };
    }).sort((a, b) => {
  ...
  } else if (salaryData && typeof salaryData === 'object' && salaryData.SalaryPeriod) {
    ...
    salaryChartData = [{
      ...
      Total: (salaryData.BaseSalary || 0) + (salaryData.Allowances || 0) + (salaryData.Bonus || 0)
    }];
  }
...
const SalaryTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <Card shadow="md" p="md" radius="md" withBorder style={{ minWidth: '200px' }}>
        <Text fw={600} size="sm" mb="md">{label}</Text>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          <Group justify="space-between">
            <Text size="sm">Lương cơ bản:</Text>
            <Text size="sm" fw={500}>{data.BaseSalary?.toLocaleString()} VNĐ</Text>
          </Group>
          <Group justify="space-between">
            <Text size="sm">Phụ cấp:</Text>
            <Text size="sm" fw={500}>{data.Allowances?.toLocaleString()} VNĐ</Text>
          </Group>
          <Group justify="space-between">
            <Text size="sm">Thưởng:</Text>
            <Text size="sm" fw={500}>{data.Bonus?.toLocaleString()} VNĐ</Text>
          </Group>
          <Box style={{ height: '1px', backgroundColor: '#e9ecef', margin: '8px 0' }} />
          <Group justify="space-between">
            <Text size="sm">Khấu trừ:</Text>
            <Text size="sm" fw={500} c="red">-{data.Deduction?.toLocaleString()} VNĐ</Text>
          </Group>
          <Group justify="space-between">
            <Text size="sm">Thuế:</Text>
            <Text size="sm" fw={500} c="red">-{data.Tax?.toLocaleString()} VNĐ</Text>
          </Group>
          <Box style={{ height: '1px', backgroundColor: '#e9ecef', margin: '8px 0' }} />
          <Group justify="space-between">
            <Text size="sm" fw={600}>Lương thực nhận:</Text>
            <Text size="sm" fw={700} c="green">{data.NetSalary?.toLocaleString()} VNĐ</Text>
          </Group>
        </div>
      </Card>
    );
...
const CustomCalendar = ({ 
  ...
  const getDayColor = (date: Date) => {
    ...
    switch (dayData.type) {
      case 'approved_leave': return '#ff6b6b'; // Red
      case 'pending_leave': return '#ffd43b'; // Yellow
      case 'overtime': return '#339af0'; // Blue
      case 'working': return '#51cf66'; // Light green
  ...
  const getDayTooltip = (date: Date) => {
    ...
    switch (dayData.type) {
      case 'approved_leave': return `Nghỉ phép đã duyệt: ${dayData.leaveType}`;
      case 'pending_leave': return `Nghỉ phép chờ duyệt: ${dayData.leaveType}`;
      case 'overtime': return `Làm OT: ${dayData.overtimeHours}h`;
      case 'working': return `Ngày làm việc: ${dayData.workHours}h`;
      default: return '';
    }
  };
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(
        <Box key={`empty-${i}`} style={{ height: '50px', border: '1px solid #e9ecef' }} />
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <Box
          ...
          title={tooltip}
        >
          <Text size="sm" fw={500} c={backgroundColor !== 'transparent' ? 'white' : 'dark'}>
            {day}
          </Text>
        </Box>
      );
...
export default function Statistical() {
  ...
  const getSalaryColor = (dataType: string) => {
    switch (dataType) {
      case 'BaseSalary': return '#339af0';
      case 'Bonus': return '#69db7c';
      case 'Total': return '#ff8cc8';
      case 'NetSalary': return '#ffd43b';
  ...
  const getSalaryLabel = (dataType: string) => {
    switch (dataType) {
      case 'BaseSalary': return 'Lương cơ bản';
      case 'Bonus': return 'Thưởng';
      case 'Total': return 'Tổng thu nhập';
      case 'NetSalary': return 'Lương thực nhận';
      default: return 'Lương thực nhận';
    }
  };
      ...
      <Tabs value={activeTab} onChange={setActiveTab} variant="pills" radius="md">
            ...
            <Grid mt="xl">
              <Grid.Col span={{ base: 12, sm: 6 }}>
                <Card padding="md" radius="md" style={{ backgroundColor: '#f8f9fa' }}>
                  <Text size="sm" c="dimmed" mb="xs">Tổng giờ làm việc</Text>
              ...
              <Grid.Col span={{ base: 12, sm: 6 }}>
                <Card padding="md" radius="md" style={{ backgroundColor: '#f8f9fa' }}>
                  <Text size="sm" c="dimmed" mb="xs">Tổng giờ OT</Text>
        ...
        <Tabs.Panel value="calendar">
          <Card shadow="sm" padding="xl" radius="md" withBorder>
            ...
            {/* Legend */}
            <Card padding="md" radius="md" style={{ backgroundColor: '#f8f9fa' }} mb="lg">
              <Text size="sm" fw={500} mb="xs">Chú thích:</Text>
              <Group gap="lg">
                <Group gap="xs">
                  <Box w={16} h={16} style={{ backgroundColor: '#ff6b6b', borderRadius: '4px' }} />
                  <Text size="sm">Nghỉ phép đã duyệt</Text>
                </Group>
                <Group gap="xs">
                  <Box w={16} h={16} style={{ backgroundColor: '#ffd43b', borderRadius: '4px' }} />
                  <Text size="sm">Nghỉ phép chờ duyệt</Text>
                </Group>
                <Group gap="xs">
                  <Box w={16} h={16} style={{ backgroundColor: '#339af0', borderRadius: '4px' }} />
                  <Text size="sm">Ngày có OT</Text>
                </Group>
                <Group gap="xs">
                  <Box w={16} h={16} style={{ backgroundColor: '#51cf66', borderRadius: '4px' }} />
          ...
          <Card shadow="sm" padding="xl" radius="md" withBorder>
            ...
            {salaryChartData.length > 0 ? (
                  ...
                  {salaryChartType === 'line' ? (
                    ...
                    >
                      ...
                      <Line
                        ...
                        strokeWidth={3}
                        dot={{ fill: getSalaryColor(salaryDataType), strokeWidth: 2, r: 6 }}
                        activeDot={{ r: 8, stroke: getSalaryColor(salaryDataType), strokeWidth: 2 }}
              ...
              <Box style={{ 
                ...
                <ThemeIcon size="xl" radius="md" color="gray" variant="light" mb="md">
                  <IconCoin size={32} />
                </ThemeIcon>
                ...
                <Grid.Col span={{ base: 12, sm: 6, lg: 3 }}>
                  <Card padding="md" radius="md" style={{ backgroundColor: '#e3f2fd' }}>
                    <Group justify="space-between" align="center" mb="xs">
                      <Text size="sm" c="dimmed">Lương TB/tháng</Text>
                      <ThemeIcon size="sm" color="blue" variant="light">
                        <IconCash size={14} />
                      </ThemeIcon>
                    </Group>
                    <Text size="lg" fw={700} c="blue">
                      {(salaryChartData.reduce((sum, item) => sum + item.NetSalary, 0) / salaryChartData.length).toLocaleString()} VNĐ
                    </Text>
                  </Card>
                </Grid.Col>
                <Grid.Col span={{ base: 12, sm: 6, lg: 3 }}>
                  <Card padding="md" radius="md" style={{ backgroundColor: '#e8f5e8' }}>
                    <Group justify="space-between" align="center" mb="xs">
                      <Text size="sm" c="dimmed">Lương cao nhất</Text>
                      <ThemeIcon size="sm" color="green" variant="light">
                        <IconTrendingUp size={14} />
                      </ThemeIcon>
                    </Group>
                    <Text size="lg" fw={700} c="green">
                      {Math.max(...salaryChartData.map(item => item.NetSalary)).toLocaleString()} VNĐ
                    </Text>
                  </Card>
                </Grid.Col>
                <Grid.Col span={{ base: 12, sm: 6, lg: 3 }}>
                  <Card padding="md" radius="md" style={{ backgroundColor: '#fff3e0' }}>
                    <Group justify="space-between" align="center" mb="xs">
                      <Text size="sm" c="dimmed">Tổng thưởng</Text>
                      <ThemeIcon size="sm" color="orange" variant="light">
                        <IconCoin size={14} />
                      </ThemeIcon>
                    </Group>
                    <Text size="lg" fw={700} c="orange">
                      {salaryChartData.reduce((sum, item) => sum + item.Bonus, 0).toLocaleString()} VNĐ
                    </Text>
                  </Card>
                </Grid.Col>
                <Grid.Col span={{ base: 12, sm: 6, lg: 3 }}>
                  <Card padding="md" radius="md" style={{ backgroundColor: '#fce4ec' }}>
                    <Group justify="space-between" align="center" mb="xs">
                    ...
                    <Text size="lg" fw={700} c="pink">
                      {salaryChartData.reduce((sum, item) => sum + item.NetSalary, 0).toLocaleString()} VNĐ
                    </Text>
                  </Card>
                </Grid.Col>
              </Grid>
